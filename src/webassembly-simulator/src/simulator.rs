...
fn calculate_next_value_for_voltage(current_of_behind_point: f64, current_of_ahead_point: f64, voltage_of_this_point: f64) -> f64 {
    let current_distance_gradient = (current_of_ahead_point - current_of_behind_point) / DISTANCESTEP_DOUBLED; // ∂/∂x(I(x, t)) ≈ (I(x + Δx, t) - I(x - Δx, t)) / 2Δx
...
fn calculate_next_lossless_value_for_voltage(current_of_behind_point: f64, current_of_ahead_point: f64, voltage_of_this_point: f64) -> f64 {
    let current_distance_gradient = (current_of_ahead_point - current_of_behind_point) / DISTANCESTEP_DOUBLED; // ∂/∂x(I(x, t)) ≈ (I(x + Δx, t) - I(x - Δx, t)) / 2Δx
    let voltage_time_gradient = (- current_distance_gradient) / TRANSMISSION_LINE_CAPACITANCE_IN_FARADS; // ∂/∂t(V(x, t)) = (- (∂/∂x(I(x, t))) - (G⋅V(x, t))) / C
    let change_in_voltage = voltage_time_gradient * TIMESTEP; // ΔV ≈ ∂/∂t(V(x, t)) * Δt
    let new_voltage = voltage_of_this_point + change_in_voltage; // V(x, t + Δt) = V(x, t - Δt) + ΔV
    return new_voltage;
}
...
fn calculate_next_value_for_current(voltage_of_behind_point: f64, voltage_of_ahead_point: f64, current_of_this_point: f64) -> f64 {
    let voltage_distance_gradient = (voltage_of_ahead_point - voltage_of_behind_point) / DISTANCESTEP_DOUBLED; // ∂/∂x(I(x, t)) ≈ (I(x + Δx, t) - I(x - Δx, t)) / 2Δx
...
fn calculate_next_lossless_value_for_current(voltage_of_behind_point: f64, voltage_of_ahead_point: f64, current_of_this_point: f64) -> f64 {
    let voltage_distance_gradient = (voltage_of_ahead_point - voltage_of_behind_point) / DISTANCESTEP_DOUBLED; // ∂/∂x(I(x, t)) ≈ (I(x + Δx, t) - I(x - Δx, t)) / 2Δx
    let current_time_gradient = (- voltage_distance_gradient) / TRANSMISSION_LINE_INDUCTANCE_IN_HENRYS; // ∂/∂t(V(x, t)) = (- (∂/∂x(I(x, t))) - (G⋅V(x, t))) / C
    let change_in_current = current_time_gradient * TIMESTEP; // ΔV ≈ ∂/∂t(V(x, t)) * Δt
    let new_current = current_of_this_point + change_in_current; // V(x, t + Δt) = V(x, t - Δt) + ΔV
    return new_current;
}
...
pub fn step_simulation() -> () {
    let mut simulation_state = SIMULATION_STATE.lock().unwrap();
    let mut next_simulation_state = SimulationState {
